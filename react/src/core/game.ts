...
import { createSetup, ref,onMounted } from 'reactivue'
...
const useGame = () => {
    ...
    const setup = createSetup(() => {
        ...
        const initGame = () => {
            // 0. 设置父容器宽高
            const levelBoardDom = document.querySelector(".level-board") as HTMLDivElement;
            if (!levelBoardDom) {
                return;
            }
            ...
            // 1. 规划块数
            // 块数单位（总块数必须是该值的倍数）
            const blockNumUnit = gameConfig.composeNum * gameConfig.materialTypeNum;
            ...
            // 需要用到的动物数组
            const needMaterialList = gameConfig.materialList.slice(0, gameConfig.materialTypeNum);
            // 依次把块塞到数组里
            for (let i = 0; i < totalBlockNum.value; i++) {
                const item = needMaterialList[i % gameConfig.materialTypeNum];
                if(item){
                    materialBlocks.push(item.value);
                }
            }

            // 打乱数组
            const randomMaterialBlocks = _.shuffle(materialBlocks);
        ...
        const doStart = () => {
            gameStatus.value = 0;
            const { levelBlocks, randomBlocks, slotArea } = initGame() as any;
